...
public interface Ord {
  ...
  @interface ApiResourceDefinition {
    ...
    /** Describes the intended purpose or role of this resource definition.  While `type` specifies the format (e.g., OpenAPI, AsyncAPI), `purpose` indicates what the definition is used for. This allows multiple definitions of the same type to coexist when they serve different purposes.  For example, an API Resource might have multiple OpenAPI definitions: one for standard API documentation and another specifically enriched for AI/agent consumption.  Together with `type` (or `customType`) and `visibility`, `purpose` forms the uniqueness key for entries in the `resourceDefinitions` list — no two entries on the same resource may share the same combination.  MUST be a valid [Concept ID](../index.md#concept-id). */
